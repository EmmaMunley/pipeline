...
`)
	...
	// The PipelineRun should be marked as failed due to ReasonResultMissing.
	if d := cmp.Diff(reconciledRun, expectedPipelineRun, ignoreResourceVersion, ignoreLastTransitionTime, ignoreTypeMeta, ignoreStartTime, ignoreCompletionTime); d != "" {
		t.Errorf("Expected to see PipelineRun run marked as failed with the reason: ReasonResultMissing. Diff %s", diff.PrintWantGot(d))
	}
