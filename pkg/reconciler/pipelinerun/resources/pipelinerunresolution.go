...
func (t ResolvedPipelineTask) isStarted() bool {
	if t.IsCustomTask() {
		return len(t.RunObjects) > 0 && t.RunObjects[0].GetStatusCondition().GetCondition(apis.ConditionSucceeded) != nil
	}
...
func (t ResolvedPipelineTask) isConditionStatusFalse() bool {
	if t.isStarted() {
		if t.IsCustomTask() {
			return t.RunObjects[0].GetStatusCondition().GetCondition(apis.ConditionSucceeded).IsFalse()
		}
...
func ResolvePipelineTask(
	...
	if rpt.PipelineTask.IsMatrixed() {
		numberOfTaskRuns = pipelineTask.Matrix.CountCombinations()
	}
	if rpt.IsCustomTask() {
		rpt.RunObjectNames = getNamesOfRuns(pipelineRun.Status.ChildReferences, pipelineTask.Name, pipelineRun.Name, numberOfTaskRuns)
...
func getNamesOfRuns(childRefs []v1beta1.ChildStatusReference, ptName, prName string, numberOfTaskRuns int) []string {
