...
func ApplyPipelineTaskContexts(pt *v1.PipelineTask, pipelineRunStatus v1.PipelineRunStatus) *v1.PipelineTask {
	...
	if looksLikeMatrixContextVar(pt.Params) {
		...
		// the number of child refs will map to the number of results produced by the matrixed PipelineTask
		if resultName != "" {
			replacements["tasks."+pipelineTaskName+".matrix."+resultName+".length"] = strconv.Itoa(len(pipelineRunStatus.ChildReferences))
		}
	}
