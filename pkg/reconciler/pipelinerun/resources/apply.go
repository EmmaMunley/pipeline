...
func ApplyTaskResults(targets []*v1beta1.PipelineTask, resolvedResultRefs ResolvedResultRefs) {
	...
	for _, resolvedPipelineRunTask := range targets {
		if resolvedPipelineRunTask != nil {
			pipelineTask := resolvedPipelineRunTask
			...
			fmt.Println("1 TEST pipelineTask", pipelineTask)
			fmt.Println("1 TEST resolvedPipelineRunTask", resolvedPipelineRunTask)
			resolvedPipelineRunTask = pipelineTask
			if resolvedPipelineRunTask.IsMatrixed() {
				fmt.Println("2 TEST", resolvedPipelineRunTask.Matrix.Params)
			}
			fmt.Println("2 TEST pipelineTask", pipelineTask)
			fmt.Println("2 TEST resolvedPipelineRunTask", resolvedPipelineRunTask)
		}
	}
}
