...
func TestMatrix(t *testing.T) {
	...
	tds := []tests{{
		...
		testSetup: func(ctx context.Context, t *testing.T, c *clients, namespace string, _ int) *v1beta1.Pipeline {
			t.Helper()
			tasks := getFanInFanOutTasksMatrix(t, namespace)
			for _, task := range tasks {
				if _, err := c.V1beta1TaskClient.Create(ctx, task, metav1.CreateOptions{}); err != nil {
					t.Fatalf("Failed to create Task `%s`: %s", task.Name, err)
				}
			...
			p := getFanInFanOutPipelineMatrix(t, namespace, tasks)
			if _, err := c.V1beta1PipelineClient.Create(ctx, p, metav1.CreateOptions{}); err != nil {
				t.Fatalf("Failed to create Pipeline `%s`: %s", p.Name, err)
			}
		...
		pipelineRunFunc:  getFanInFanOutPipelineRunMatrix,
		expectedTaskRuns: []string{"platforms-and-browsers"},
		// 1 from PipelineRun and 4 from Tasks defined in pipelinerun
		expectedNumberOfEvents: 9,
	}}

	var requiredAlphaFeatureFlags = requireAnyGate(map[string]string{
	...
	for i, td := range tds {
		...
		t.Run(td.name, func(t *testing.T) {
			...
			c, namespace := setup(ctx, t, requiredAlphaFeatureFlags)
			knativetest.CleanupOnInterrupt(func() { tearDown(ctx, t, c, namespace) }, t.Logf)
			defer tearDown(ctx, t, c, namespace)
...
func getFanInFanOutTasksMatrix(t *testing.T, namespace string) map[string]*v1beta1.Task {
	t.Helper()
	return map[string]*v1beta1.Task{
		"platforms-and-browsers": parse.MustParseV1beta1Task(t, fmt.Sprintf(`
metadata:
  name: %s
  namespace: %s
spec:
...
func getFanInFanOutPipelineMatrix(t *testing.T, namespace string, tasks map[string]*v1beta1.Task) *v1beta1.Pipeline {
	t.Helper()
	return parse.MustParseV1beta1Pipeline(t, fmt.Sprintf(`
metadata:
  name: %s
      ...
      params:
        ...
        - name: browser
          value:
            ...
            - safari
            - firefox
`, helpers.ObjectNameForTest(t), namespace, tasks["platforms-and-browsers"].Name))
}

func getFanInFanOutPipelineRunMatrix(t *testing.T, _ int, namespace string, pipelineName string) *v1beta1.PipelineRun {
	t.Helper()
	return parse.MustParseV1beta1PipelineRun(t, fmt.Sprintf(`
metadata:
  name: %s
