...
func TestTaskResultsFromMatrix(t *testing.T) {
...
spec:
  pipelineSpec:
          ...
          name: configurable-exit-code
				taskSpec:
					...
					- name: echo
						image: alpine
						script: |
							exit "$(params.exit-code)`, helpers.ObjectNameForTest(t)))
	...
	if len(taskrunList.Items) != 2 {
		t.Fatalf("The pipelineRun \"%s\" should have exactly 2 taskRuns"+
			"instead it has \"%d\" taskRuns", pipelineRun.Name, len(taskrunList.Items))
	}
	...
	for _, taskrunItem := range taskrunList.Items {
		...
		case "task1":
			...
			for _, r := range taskrunItem.Status.TaskRunResults {
				if r.Name == "result1" && r.Value.StringVal != "123" {
					t.Fatalf("task1 should have exited with code 0")
				}
			}
		case "finaltask1":
		...
		default:
			t.Fatalf("TaskRuns were not found for matrixed tasks")
		}
	}
}
